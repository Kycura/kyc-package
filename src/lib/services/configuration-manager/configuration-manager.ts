...
export const mergeConfigOverrides = (overrides: FlowsInitOptions): IAppConfiguration => {
  ...
  configurationStore.update(configuration => {
    ...
    // First merge all config except uiConfig
    mergedConfiguration = deepmerge(configuration, restConfig) as IAppConfiguration;
    
    // Special handling for backendConfig to ensure proper merging of nested objects
    if (overrides.backendConfig) {
      mergedConfiguration.backendConfig = {
        ...mergedConfiguration.backendConfig,
        ...overrides.backendConfig,
        // Merge auth config properly
        auth: overrides.backendConfig.auth 
          ? { ...mergedConfiguration.backendConfig.auth, ...overrides.backendConfig.auth }
          : mergedConfiguration.backendConfig.auth,
        // Merge endpoints properly
        endpoints: overrides.backendConfig.endpoints
          ? { ...mergedConfiguration.backendConfig.endpoints, ...overrides.backendConfig.endpoints }
          : mergedConfiguration.backendConfig.endpoints,
      };
    }
    
    // Then merge uiConfig if provided
    if (uiConfig) {
