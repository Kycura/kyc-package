...
import { KYCURA_EVENT } from './constants';
import { IEventOptions, IFlowErrorPayload, IFlowExitPayload } from '../../services/flow-event-bus/interfaces';
...
export const sendFlowExitEvent = (actionName: TActionNames, status: IDocumentVerificationResponse) => {
  const as = get(appState);
  const eventOptions: IFlowExitPayload = {
    eventName: KYCURA_EVENT,
...
export const sendButtonClickEvent = (
  ...
  // If shouldExit is true, also trigger the flow exit event
  if (shouldExit) {
    sendFlowExitEvent(actionName, status);
  }
};
