...
import type { KycuraSDKFlows, FlowsInitOptions } from './types/KycuraSDK';
...
export * from './types/KycuraSDK';

export const flows: KycuraSDKFlows = {
  init: config => {
    return new Promise((resolve, reject) => {
      console.log(
        'KycuraSDK: Initializing KycuraSDK with version: ',
        __APP_VERSION__,
        config,
      );
  ...
  mount({
    ...
    if (hostElement) {
      ...
      console.error(`KycuraSDK: Could not find element ${message}`);
    }
